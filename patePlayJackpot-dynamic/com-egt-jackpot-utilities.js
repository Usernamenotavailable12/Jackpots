...
function maskUsername(username) {
  ...
  function formatData(value) {
	...
	// Round to 2 decimal places and convert to integer by removing decimal point
...
function RollingComponent(h, w, d, b) {
	...
	f.append = function () {
		...
		var valueContainer = f.box.find('.com-egt-jackpot-html-value');
		if(valueContainer.length) {
			valueContainer.append(f.currency);
		}
	...
	EgtJackpotComponent = function () {
		...
		H.prototype.onRequestComplete = function (e) {
			...
			let biggest = e.result.pools[0]

			smallest.amount = formatData(smallest.amount);
			small.amount = formatData(small.amount);
			big.amount = formatData(big.amount);
			biggest.amount = formatData(biggest.amount);

			u.push(smallest.amount);
			u.push(small.amount);
			u.push(big.amount);
		...
		H.prototype.processRequestFromServer = function (a) {
			let smallest = a.result.pools[3]
			let small = a.result.pools[2]
			let big = a.result.pools[1]
			let biggest = a.result.pools[0]
