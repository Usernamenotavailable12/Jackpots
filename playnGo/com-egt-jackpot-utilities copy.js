...
var indexOfGame = [3213, 3212, 3211, 3210];
...
function findJackpotById(jackpots, targetId) {
    let jackpotsArray = Array.isArray(jackpots) ? jackpots : Object.values(jackpots);
...
function RollingComponent(h, w, d, b) {
	...
	c = '<div class="com-egt-jackpot-html-currency">' + CURRENCY_TEXT + "</div>";
	...
	h.append(f.box.addClass("com-egt-jackpot-html-" + w));
	...
	f.level.attr("width", d.levelSize);
	f.level.attr("height", d.levelSize);
	f.generateLevels(w, d.levelSize);
	f.currency = $(c).css({
		lineHeight: d.boxHeight / 4 + "px", fontSize: d.boxHeight /
			6 + "px"
	});
	f.currency.children().css({ paddingTop: d.boxHeight / 8 + "px" });
	...
	for (h = 0; h < b.maxDigits; h++) {
		w = $('<span class="com-egt-jackpot-html-old"></span>').css({ height: height + "px" });
		d = $('<span class="com-egt-jackpot-html-new"></span>').css({ height: height + "px" });
	...
	function h() {
		var a = d.width();
		d.removeClass();
		a < l && d.toggleClass("com-egt-jackpot-html-xsm");
		a >= l && a < g && d.toggleClass("com-egt-jackpot-html-sm");
		a >= g && a < n && d.toggleClass("com-egt-jackpot-html-md");
		a >= n && a < e && d.toggleClass("com-egt-jackpot-html-lg");
		a >= e && d.toggleClass("com-egt-jackpot-html-xlg")
	}
	...
	EgtJackpotComponent = function () {
		function H(a) {
			...
			if (E) z = "Oops your browser is not supported!", this.appendIE8Box(a.boxHeight); else {
				...
				this.Heart;
				this.Spade;
				a.logo && (a.redirectURL ? this.createLink(d).attr("href", a.redirectURL).addClass("com-egt-jackpot-html-logo com-egt-jackpot-html-mobile").css({
					height: a.boxHeight +
						"px"
				}) : this.createDiv(d).addClass("com-egt-jackpot-html-logo com-egt-jackpot-html-mobile").css({ height: a.boxHeight + "px" }));
				...
				a.logo && (a.redirectURL ? this.createLink(d).attr("href", a.redirectURL).addClass("com-egt-jackpot-html-logo").css({ height: a.boxHeight + "px" }) : this.createDiv(d).addClass("com-egt-jackpot-html-logo").css({ height: a.boxHeight + "px" }));
				this.Diamond =
		...
		H.prototype.setLanguageFromDefaultJson =
			function (a) {
				isLanguageSet = !1;
				c ? (c = c.toUpperCase(), a.LargestWinner.hasOwnProperty(c) && (B.topWinText.html(a.LargestWinner[c]), B.countWinsText.html(a.NumberOfWinners[c]), isLanguageSet = !0), 0 == isLanguageSet && (B.topWinText.html(a.LargestWinner.EN), B.countWinsText.html(a.NumberOfWinners.EN), B.lastWinText.html(a.LastWinner.EN))) : (B.topWinText.html(a.LargestWinner.EN), B.countWinsText.html(a.NumberOfWinners.EN), null)
			};
		...
		H.prototype.onRequestComplete = function (e) {
			...
			n.first = !1;
			a = e.currency;
			let smallest = findJackpotById(e, indexOfGame[0]);
			let small = findJackpotById(e, indexOfGame[1]);
			let big = findJackpotById(e, indexOfGame[2]);
			let biggest = findJackpotById(e, indexOfGame[3]);

			u.push(smallest.BaseAmount);
		...
		H.prototype.processRequestFromServer = function (a) {
			let smallest = findJackpotById(a, indexOfGame[0]); 
			let small = findJackpotById(a, indexOfGame[1]);
			let big = findJackpotById(a, indexOfGame[2]); 
			let biggest = findJackpotById(a, indexOfGame[3]);

			u[0] = smallest.BaseAmount;
			u[1] = small.BaseAmount;
			u[2] = big.BaseAmount;
			u[3] = biggest.BaseAmount;
		...
		H.prototype.checkUserNameLength = function (a) {
			if (!a) return "";
			return a.length > 17 ? a.slice(0, 17) + "..." : a;
		};
