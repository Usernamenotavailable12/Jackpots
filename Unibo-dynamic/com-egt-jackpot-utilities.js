...
function RollingComponent(h, w, d, b) {
	...
	f.box.css({
		...
		borderStyle: "solid",
		borderColor: getBorderColor(f.id),
	});
	...
	f.currency = $(sadasdlas).css({
		lineHeight: d.boxHeight / 4 + "px", fontSize: 12 + "px" });
	...
	EgtJackpotComponent = function () {
		...
		H.prototype.onRequestComplete = function (e) {
			...
			n.first = !1;
			a = "&#8382;";
			u.push(e.jackpots[1].max_payout_amount > e.jackpots[0].max_payout_amount ? Math.round(parseFloat(e.jackpots[1].amount.replace("GEL", "").replace(",", "").trim()) * 100) : Math.round(parseFloat(e.jackpots[0].amount.replace("GEL", "").replace(",", "").trim()) * 100));
			u.push(e.jackpots[1].max_payout_amount < e.jackpots[0].max_payout_amount ? Math.round(parseFloat(e.jackpots[1].amount.replace("GEL", "").replace(",", "").trim()) * 100) : Math.round(parseFloat(e.jackpots[0].amount.replace("GEL", "").replace(",", "").trim()) * 100));
		...
		H.prototype.processRequestFromServer = function (a) {
			...
			u[0] = a.jackpots[1].max_payout_amount > a.jackpots[0].max_payout_amount ? Math.round(parseFloat(a.jackpots[1].amount.replace("GEL", "").replace(",", "").trim()) * 100) : Math.round(parseFloat(a.jackpots[0].amount.replace("GEL", "").replace(",", "").trim()) * 100);
			u[1] = a.jackpots[1].max_payout_amount < a.jackpots[0].max_payout_amount ? Math.round(parseFloat(a.jackpots[1].amount.replace("GEL", "").replace(",", "").trim()) * 100) : Math.round(parseFloat(a.jackpots[0].amount.replace("GEL", "").replace(",", "").trim()) * 100);
